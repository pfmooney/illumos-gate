    ...
    struct vm_exit *vexit)
{
	...
	const uint_t margin_ticks = MAX(1, LAPIC_TARGET_TICKS / 10000);
	...
	do {
		...
		if (vexit->exitcode == VM_EXITCODE_INOUT &&
		    vexit->u.inout.port == IOP_TEST_PARAM &&
		    vexit->u.inout.bytes == 4 &&
		    (vexit->u.inout.flags & INOUT_IN) != 0) {
			bcopy(&vexit->u.inout, &ventry->u.inout,
			    sizeof (struct vm_inout));
			ventry->cmd = VEC_FULFILL_INOUT;
			ventry->u.inout.eax = divisor;
			continue;
		}

		if (vexit->exitcode == VM_EXITCODE_INOUT &&
		    vexit->u.inout.port == IOP_TEST_VALUE &&
		    vexit->u.inout.bytes == 4 &&
		    (vexit->u.inout.flags & INOUT_IN) == 0) {
			...
			/*
			 * Allow interval to be up to 7 ticks extra, and/or
			 * 400ppm off in equivalent time.
			 */
