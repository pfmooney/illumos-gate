...
dls_promisc(dld_str_t *dsp, uint32_t new_flags)
{
	...
	uint32_t new_type = new_flags & ~(DLS_PROMISC_FIXUPS);
	...
	ASSERT(!(new_flags & ~(DLS_PROMISC_SAP | DLS_PROMISC_MULTI |
	    DLS_PROMISC_PHYS | DLS_PROMISC_FIXUPS)));

	/*
	 * Asking us just to turn on only DLS_PROMISC_FIXUPS is not valid.
	 */
	if ((new_flags & ~(DLS_PROMISC_FIXUPS)) == 0 && new_flags != 0)
		return (EINVAL);
