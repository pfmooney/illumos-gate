 ...
 * cb_data: passed to cb_wait calls
 */
int
page_xresv(pgcnt_t npages, uint_t flags, int (*cb_wait)(void *), void *cb_data)
{
	int bail = 1;

	mutex_enter(&freemem_lock);
	while (availrmem < tune.t_minarmem + npages) {
		if ((flags & KM_NOSLEEP) != 0 || bail == 0) {
			mutex_exit(&freemem_lock);
			return (0);
		}
		mutex_exit(&freemem_lock);
...
{
	return (page_xresv(npages, flags, NULL, NULL));
}
