    ...
    cred_t *credp, int *rvalp)
{
	...
	case VM_DATA_READ: {
		...
		void *buf = kmem_alloc(vdx.vdx_len, KM_SLEEP);
		if ((vdx.vdx_flags & VDX_FLAG_READ_COPYIN) != 0) {
			if (ddi_copyin(vdx.vdx_data, buf, vdx.vdx_len, md) != 0) {
				kmem_free(buf, vdx.vdx_len);
				error = EFAULT;
				break;
			}
		} else {
			bzero(buf, vdx.vdx_len);
		}
		...
		if (error == 0) {
			if (ddi_copyout(buf, vdx.vdx_data, vdx.vdx_len, md) != 0) {
				error = EFAULT;
			}
		}
		kmem_free(buf, vdx.vdx_len);
	...
	case VM_DATA_WRITE: {
		...
		if (error == 0 && (vdx.vdx_flags & VDX_FLAG_WRITE_COPYOUT) != 0) {
			if (ddi_copyout(buf, vdx.vdx_data, vdx.vdx_len, md) != 0) {
				error = EFAULT;
			}
		}
