...
struct vdi_lapic {
	struct vdi_lapic_page	vl_lapic;
	uint64_t		vl_msr_apicbase;
	uint64_t		vl_timer_target;
...
struct vdi_atpit_channel {
	...
	uint64_t	vac_time_loaded;
	uint64_t	vac_time_target;
};

struct vdi_atpit {
	struct vdi_atpit_channel va_channel[3];
};
...
struct vdi_atpic {
	struct vdi_atpic_chip va_chip[2];
};


/* VDC_HPET: */

struct vdi_hpet_timer {
	uint64_t	vht_config;
	uint64_t	vht_msi;
	uint32_t	vht_comp_val;
	uint32_t	vht_comp_rate;
	uint64_t	vht_time_base;
};
...
struct vdi_hpet {
	...
	uint64_t	vh_time_base;

	struct vdi_hpet_timer	vh_timers[8];
};

/* VDC_PM_TIMER: */

struct vdi_pm_timer {
	uint64_t	vpt_time_base;
	uint32_t	vpt_val_base;
	uint16_t	vpt_ioport;
};
