 ...
 * left-most child, and its leftward/rightward siblings.  The node understands
 * its position in the tree in terms of its level it appears at and the index it
 ...
 * leftward/rightward siblings.  Interior nodes also maintain a reference count,
 * and each node contains its level and index in its parent's table.  Finally,
...
/* Get length of VA covered by entries at a given table level. */
static inline uint64_t
...
vmm_gpt_node_next(vmm_gpt_node_t *node, bool only_seq)
{
	...
	if (next != NULL) {
		...
		/* Otherwise check first child of parent's next sibling */
...
/*
 * Add a child node to the GPT at a given `gpa` and (implied by parent) level.
 */
static void
...
/*
 * Removes a child node from its relatives (parent, siblings) and frees it.
 */
static void
...
vmm_gpt_populate_region_lvl(vmm_gpt_t *gpt, uint64_t addr, uint64_t len,
    vmm_gpt_node_t *parent_start)
{
	const vmm_gpt_node_level_t lvl = parent_start->vgn_level;
	const uint64_t end = addr + len;
	const uint64_t incr = vmm_gpt_lvl_len(lvl);
	uint64_t gpa = addr & vmm_gpt_lvl_mask(lvl);
	vmm_gpt_node_t *parent = parent_start;
	...
	/*
	 * If not node exists at the starting address, create one and link it
	 * into the parent.
	 */
